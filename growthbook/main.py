...
def process_config_element(config_key, config_value, growthbook_client):
    ...
    default_value, rules, value_type, attributes_needed, skipped_keys = growthbook_client.process_config_to_rules(
        config_value, 
...
if __name__ == "__main__":
    ...
    config_file = os.path.join(current_dir, 'configuration.yaml')
    
    # Replace with your actual GrowthBook API key
    GROWTHBOOK_API_KEY = "secret_user_a5Qq7D1rzZeKS17yffkE0xhYYXprCOl71S6OZUUQmxk"
    GROWTHBOOK_PROJECT = "prj_7gxlgu1nmkjtaucm"
    GROWTHBOOK_OWNER = "nam.nguyen@sotatek.com"
    growthbook_client = GrowthBook(api_key=GROWTHBOOK_API_KEY, project=GROWTHBOOK_PROJECT, owner=GROWTHBOOK_OWNER)
